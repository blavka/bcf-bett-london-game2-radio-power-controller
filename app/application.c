...
void led_strip_fill(void)
{
    ...
    else if (led_strip.show == LED_STRIP_SHOW_MORSE_CODE)
    {
        bc_led_strip_fill(&led_strip.self, 0);

        int position = 0;
        int j;
        for (size_t i = 0; i < sizeof(led_strip.morse_code); i++) {
            if (led_strip.morse_code[i] == '.') {
                for (j = 0; j < 3; j++) {
                    bc_led_strip_set_pixel(&led_strip.self, position, 0xff000000);
                    position++;
                }
            }
            else if (led_strip.morse_code[i] == '-') {
                for (j = 0; j < 6; j++) {
                    bc_led_strip_set_pixel(&led_strip.self, position, 0xff000000);
                    position++;
                }
                ...
                for (j = 0; j < 3; j++) {
                    bc_led_strip_set_pixel(&led_strip.self, position, 0x00000000);
                    position++;
                }
            }
...
void led_strip_morse_code_set(uint64_t *id, const char *topic, void *value, void *param)
{
    bc_led_pulse(&led, 30);

    bc_led_strip_effect_stop(&led_strip.self);

    strncpy(led_strip.morse_code, (char *) value, sizeof(led_strip.morse_code));

    led_strip.show = LED_STRIP_SHOW_MORSE_CODE;
...
static const bc_radio_sub_t subs[] = {
    // state/set
    {"led-strip/-/morse-code/set", BC_RADIO_SUB_PT_STRING, led_strip_morse_code_set, NULL},
};
...
void application_init(void)
{
    ...
    bc_radio_pairing_request("game2-power-controller", VERSION);
